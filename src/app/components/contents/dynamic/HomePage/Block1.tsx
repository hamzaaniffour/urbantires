    ...
    <section className="max-w-[90%] sm:max-w-[95%] md:max-w-[1000px] lg:max-w-[1000px] xl:max-w-[1250px] mx-auto mb-14" aria-label="Recipes by Category Section">
      <div className="flex justify-center items-center flex-col">
        <h2 className="text-violet-950 font-bold mb-5 text-3xl text-center uppercase">
          Tires by{" "}
          <span className="bg-blue-800 font-semibold rounded text-white py-0.5 px-2">
            Category
          </span>
        </h2>
            ...
            <Link href={`/${category.slug}`} key={category.slug} aria-label={`Explore recipes in ${category.name} category`}>
              <div className="relative overflow-hidden rounded-lg">
                <Image
                  ...
                  layout="responsive"
                />
                <div className={`absolute inset-0 text-blue-950 transition-all hover:text-blue-800 bg-gradient-to-t from-slate-600/45 to-transparent flex justify-center items-center`}>
                  <h3 className="bg-white px-4 py-1 rounded text-xl font-semibold z-40 relative">
                    {category.name}
                  </h3>
                </div>
              </div>
